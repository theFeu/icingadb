...
//Counter on how many host/service is_overdue have synced since the last logOverdueSyncCounters()
...
func startOverdueSync(super *supervisor.Supervisor) {
	go syncOverdue(super, "host", updateHostOverdue)
	go syncOverdue(super, "service", updateServiceOverdue)
	go logOverdueSyncCounters()
}

//Logs the amount of synced is_overdue every 20 seconds
...
//Tries to sync is_overdue of given object type every second
func syncOverdue(super *supervisor.Supervisor, objectType string, observer prometheus.Observer) {
	for super.EnvId == nil {
	...
	for {
		...
		updateOverdue(super, objectType, observer, root[0].([]interface{}), true)
		updateOverdue(super, objectType, observer, root[1].([]interface{}), false)
...
func updateOverdue(super *supervisor.Supervisor, objectType string, observer prometheus.Observer, ids []interface{}, overdue bool) {
	if len(ids) > 0 {
